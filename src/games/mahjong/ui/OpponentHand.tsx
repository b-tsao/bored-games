...
export function OpponentHand(props) {
    ...
    const [hand, setHand]: [React.ComponentType[], AnyFunction] = useState([]);
    ...
    useEffect(() => {
        setConcealed(
            props.hand.concealed.map((_, setIndex) => {
                return new Array(4).fill(null).map((_, tileIndex) => {
                    return (
    ...
    useEffect(() => {
        setHand(
            new Array(props.hand.hand).fill(null).map((_, tileIndex) => {
                return (
                    <Box key={tileIndex}>
                        <TileBack />
                    </Box>
