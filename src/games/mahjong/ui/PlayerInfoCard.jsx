...
export function PlayerInfoCard(props) {
  ...
  return (
    <Box display="flex" justifyContent="space-between" px={15} className={classes.root}>
      <Box className={classes.icons}>
        {props.dice === true && <CasinoTwoToneIcon />}
        {props.dealer === true && <PlayArrowRoundedIcon />}
      </Box>
      <Box display="flex" alignItems="center">
        <Avatar variant="square">{props.name ? props.name.charAt(0) : "?"}</Avatar>
        <Box ml={2}>
